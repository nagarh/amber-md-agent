...
def mark_disulfides(pdb, cutoff=2.5):
    """
    Detect disulfide bonds by SG-SG distance (< cutoff Å), rename both CYS -> CYX
    ...
    M-41: cutoff parameterized; standard SS bond = 2.05 Å, accept up to 2.5 Å default.
    """
    import MDAnalysis as mda
    import numpy as np
    import warnings
    ...
    if len(sg) < 2:
        return []  # M-41: guard against <2 SG atoms (no possible disulfide)
    pairs = []
    for i in range(len(sg)):
        for j in range(i + 1, len(sg)):
            if np.linalg.norm(sg[i].position - sg[j].position) < cutoff:
