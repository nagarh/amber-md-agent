...
def run_cmd(cmd, cwd=None, check=True, capture=True, timeout=None):
    """Run a shell command or argv list. Returns CompletedProcess.

    C-04 fix: when cmd is a list, shell=False is used to prevent shell injection.
    When cmd is a string, shell=True is used for backward compatibility with
    internal callers that use pipes or shell built-ins. Callers with user-
    supplied paths should pass a list to avoid injection.
    """
    logger.info(f"$ {cmd}")
    use_shell = not isinstance(cmd, list)
    result = subprocess.run(
        cmd, shell=use_shell, cwd=cwd, check=False,
...
def fetch_pdb(pdb_id, output_dir="."):
    ...
    if not out.exists():
        raise RuntimeError(f"Failed to download PDB {pdb_id} — file missing")
    # M-28: rely on ATOM-record check, not size (404 HTML pages exceed 1000 bytes)
    content = out.read_text(errors='ignore')
    if 'ATOM' not in content and 'HETATM' not in content:
        raise RuntimeError(f"Downloaded file for {pdb_id} contains no ATOM/HETATM records (likely 404)")
...
def inspect_pdb(pdb_file):
    ...
    with open(pdb_file) as f:
        for line in f:
            if line.startswith(("ATOM", "HETATM")):
                ...
                if atomname.startswith("H") or atomname in ("1H", "2H", "3H") or re.match(r'^\d+H', atomname):
    ...
    # Parse DBREF records → uniprot_per_chain {chain: uniprot_id}
    # DBREF format: cols 12=chain, 26-33=database, 34-41=db-accession
    # e.g. "DBREF  1UIJ B    1   416  UNP    P25974   GLCB_SOYBN ..."
    uniprot_per_chain = {}
    with open(pdb_file) as f:
        for line in f:
            ...
            try:
                ...
                if chain and db == "UNP" and acc:
                    uniprot_per_chain[chain] = acc
            except IndexError:
                pass
    info["uniprot_per_chain"] = uniprot_per_chain

    # Coordinate continuity check: find gaps in ATOM record residue numbering per chain.
    # Catches missing regions not documented in REMARK 465 (e.g. silently absent loops).
    chain_resseqs = {}
    with open(pdb_file) as f:
        for line in f:
            ...
            if icode:  # skip insertion-code residues (non-standard numbering)
                continue
            try:
                resseq = int(line[22:26])
            except ValueError:
                continue
            if chain not in chain_resseqs:
                chain_resseqs[chain] = set()
            chain_resseqs[chain].add(resseq)

    coordinate_gaps = []
    for chain in sorted(chain_resseqs):
        nums = sorted(chain_resseqs[chain])
        for i in range(len(nums) - 1):
            ...
            if gap_start <= gap_end:
                coordinate_gaps.append({
                    ...
                    "start": gap_start,
                    "end":   gap_end,
                    "n_residues": gap_end - gap_start + 1
                })
    info["coordinate_gaps"] = coordinate_gaps
...
def clean_pdb(pdb_file, output_file=None, keep_waters=False, keep_hydrogens=False):
    ...
    if not keep_waters:
        flags.append("--dry")
    # H-02 fix: --reduce runs `reduce` to ADD H, not remove them. Never pass it for H-stripping.
    # H-stripping is done as a post-processing step below.
    flags.append("--no-conect")

    # C-04 fix: use list form (shell=False) — pdb_file/output_file may contain spaces or metacharacters
    cmd = ["pdb4amber", "-i", str(pdb_file), "-o", str(output_file)] + flags
    try:
        result = run_cmd(cmd, check=False)
    except (FileNotFoundError, OSError):
        # pdb4amber not in PATH (e.g. login node) — fall through to manual cleanup below
        result = None
    ...
    # H-02 fix: post-process H-stripping. pdb4amber does not reliably strip H from NMR/high-res
    # structures. Always strip when keep_hydrogens=False, regardless of pdb4amber behavior.
    if not keep_hydrogens and output_file.exists():
        import re as _re
        _H_ATOM = _re.compile(r'^[0-9]*H')
        in_lines = output_file.read_text().splitlines(keepends=True)
        with open(output_file, 'w') as _fh:
            for _line in in_lines:
                if _line.startswith(('ATOM', 'HETATM')):
                    _aname = _line[12:16].strip()
                    if _H_ATOM.match(_aname):
                        continue
                _fh.write(_line)
...
def _validate_restraintmask(mask):
    """Raise ValueError if mask would select TIP3P water oxygens."""
    import re
    for match in re.finditer(r'@([A-Z0-9,]+)', mask):
        atom_names = match.group(1).split(',')
        if 'O' in atom_names:
            raise ValueError(
                f"restraintmask '{mask}' contains atom name 'O' which selects TIP3P water "
                "oxygens (atom name 'O' in TIP3P). Use '@CA,C,N' for backbone restraints. "
                "See skills/amber-bugs.md: '@CA,C,N,O backbone mask restrains TIP3P water oxygens'."
            )
...
def write_mdin(output_path, namelist_params, title="Generated by AmberMD Agent",
    ...
    """
    mask = namelist_params.get("restraintmask", "")
    if mask:
        _validate_restraintmask(mask)

    lines = [title, " &cntrl"]
    for key, val in namelist_params.items():
        if isinstance(val, str):
            # L-01: strip outer quotes if caller already quoted (avoid restraintmask=''@CA'')
            v = val.strip()
...
def write_cpptraj(output_path, commands):
    ...
    if isinstance(commands, list):
        ...
        content = commands
    if not re.search(r'\brun\b', content, re.IGNORECASE):
        content = content.rstrip() + "\nrun\n"
    if not re.search(r'\bquit\b', content, re.IGNORECASE):
        content = content.rstrip() + "\nquit\n"
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    Path(output_path).write_text(content)
...
def run_amber(engine, mdin, mdout, prmtop, inpcrd, rst_out, traj_out=None,
    ...
    if extra_flags:
        # C-04 fix: accept extra_flags as list (preferred) or split string via shlex
        if isinstance(extra_flags, list):
            parts.extend(extra_flags)
        else:
            parts.extend(shlex.split(extra_flags))

    # C-04 fix: pass as list (shell=False) so argv is not shell-interpreted
    start = time.time()
    result = run_cmd(parts, cwd=cwd, check=False, timeout=timeout)
    elapsed = time.time() - start

    # H-15 fix: resolve rst_out against cwd (or CWD if cwd is None) so relative
    # paths don't silently resolve against the agent's working directory.
    rst_path = Path(cwd or Path.cwd()) / rst_out
...
def write_slurm_script(output_path, commands, job_name="amber_md",
    ...
    # H-01 fix: strip module load / amber source lines from cmd_block.
    # The SLURM template (env_lines) already provides all module loads in the script header.
    # If the caller duplicates them in commands=, it causes double-loading → openmpi4 conflict.
    import re as _re
    _MOD_RE = _re.compile(
        r'^\s*(module\s+(load|unload|purge|switch|use|avail)\b.*|source\s+.*/amber\.sh\b.*)\s*$',
        _re.IGNORECASE
    )
    cmd_block = "\n".join(l for l in cmd_block.splitlines() if not _MOD_RE.match(l))
...
def submit_slurm(script_path, cwd=None):
    ...
    # Parse job ID from "Submitted batch job 12345" (M-31: canonical pattern first)
    if result.stdout:
...
def wait_for_slurm_job(job_id, poll_interval=5, timeout=3600):
    """Block until SLURM job completes. Poll every poll_interval seconds.

    Returns {"state": "COMPLETED"|"FAILED"|"TIMEOUT", "elapsed_s": N}.
    Use for short prep jobs (tLEaP, antechamber) — avoids LLM round-trip per poll.
    Default poll_interval=5s (fast for CPU prep jobs). Set higher for GPU MD runs.
    """
    ...
    while True:
        ...
        if elapsed > timeout:
            return {"state": "TIMEOUT", "elapsed_s": int(elapsed),
                    "job_id": str(job_id)}
        # Check squeue first (job still running/pending)
        sq = run_cmd(f"squeue -j {job_id} -h -o '%T'", check=False)
        state_sq = sq.stdout.strip() if sq.stdout else ""
        if state_sq in ("RUNNING", "PENDING", "CONFIGURING", "COMPLETING"):
            _time.sleep(poll_interval)
            continue
        # Job left squeue — check sacct for final state
        sa = run_cmd(
            f"sacct -j {job_id} --format=State --noheader -P",
            check=False
        )
        lines = [l.strip() for l in (sa.stdout or "").splitlines() if l.strip()]
        # sacct may return "COMPLETED", "FAILED", "CANCELLED", "TIMEOUT" etc.
        if lines:
            final = lines[0].split()[0]
        elif "accounting storage is disabled" in (sa.stderr or "").lower():
            # SLURM accounting disabled — job left squeue so it finished; validate_tleap catches failures
            final = "COMPLETED"
        else:
            final = "UNKNOWN"
        return {"state": final, "elapsed_s": int(elapsed), "job_id": str(job_id)}
...
def check_convergence(data_file, column=1, abs_threshold=0.5):
    ...
    try:
        ...
        drift_pct = (drift_abs / abs(mean) * 100) if abs(mean) > 1e-9 else float('inf')
...
def preflight(pdb_file, check_ligands=True):
    ...
    for chain, term in chain_termini.items():
        ...
        # M-33: require BOTH caps (or starts at residue 1) — half-capped chain still has bare terminus
        if has_ace and has_nme:
            already_capped = True
            report["checks"].append({
                "check": f"termini_{chain}",
                "status": "PASS",
                "detail": f"Chain {chain}: fully capped (ACE + NME)",
            })
        elif has_ace ^ has_nme:
            needs_capping = True
            report["checks"].append({
                "check": f"termini_{chain}",
                "status": "WARN",
                "detail": f"Chain {chain}: half-capped — has {'ACE' if has_ace else 'NME'} but missing {'NME' if has_ace else 'ACE'} (M-33: bare terminus)",
            })
        elif term["first"] > 1:
            needs_capping = True
            report["checks"].append({
    ...
    chain_resnums = {}
    atom_coords = {}  # (chain, resnum, atom_name) → (x, y, z)
    with open(pdb_file) as f:
        for line in f:
            if line.startswith("ATOM"):
                ...
                # Collect backbone atom coordinates for C→N distance check
                ...
                if atom_name in ("C", "N"):
                    try:
                        ...
                        y = float(line[38:46])
                        z = float(line[46:54])
                        # Keep first occurrence (lowest serial) per (chain, resnum, atom_name)
                        key = (chain, resnum, atom_name)
                        if key not in atom_coords:
                            atom_coords[key] = (x, y, z)
                    except ValueError:
                        pass

    breaks_found = []
    numbering_gaps = []  # backbone-continuous gaps (WARN only)
    for chain, resnums in chain_resnums.items():
        ...
        for i in range(1, len(sorted_res)):
            ...
            if gap > 1:
                # Check backbone C→N distance to distinguish real break from numbering artifact
                c_key = (chain, sorted_res[i-1], "C")
                n_key = (chain, sorted_res[i], "N")
                c_xyz = atom_coords.get(c_key)
                n_xyz = atom_coords.get(n_key)
                backbone_continuous = False
                cn_distance = None
                if c_xyz is not None and n_xyz is not None:
                    cn_distance = math.sqrt(
                        (c_xyz[0] - n_xyz[0]) ** 2 +
                        (c_xyz[1] - n_xyz[1]) ** 2 +
                        (c_xyz[2] - n_xyz[2]) ** 2
                    )
                    if cn_distance < 2.0:
                        backbone_continuous = True
                ...
                if backbone_continuous:
                    numbering_gaps.append(entry)
                else:
                    breaks_found.append(entry)
    ...
    if breaks_found:
        ...
        # All gaps are numbering artifacts — backbone is continuous
        detail_strs = [
            f"Chain {b['chain']}: numbering gap {b['from']}→{b['to']} — backbone continuous "
            f"(C→N {b['cn_distance']:.2f} Å)"
            for b in numbering_gaps
        ]
        report["checks"].append({
            "check": "chain_breaks",
            "status": "WARN",
            "detail": f"{len(numbering_gaps)} numbering gap(s) — backbone continuous (non-sequential PDB numbering)",
            "breaks": detail_strs,
        })
    ...
    # Numbering gaps that coexist with real breaks are reported separately as WARNs
    if breaks_found and numbering_gaps:
        detail_strs = [
            f"Chain {b['chain']}: numbering gap {b['from']}→{b['to']} — backbone continuous "
            f"(C→N {b['cn_distance']:.2f} Å)"
            for b in numbering_gaps
        ...
        report["checks"].append({
            ...
            "status": "WARN",
            "detail": f"{len(numbering_gaps)} additional numbering gap(s) with continuous backbone — no action needed",
            "breaks": detail_strs,
        })
...
def validate_step(mdout_file, expected_nstep=None, min_density=None,
    ...
    # --- Check 2: Final NSTEP ---
    # MD format:   "NSTEP =   500000   TIME(PS) = ..."
    # Minimization: "   5000  -1.78234E+05  5.39437E+00 ..." (table, no "=" sign)
    nstep_matches = re.findall(r'NSTEP\s+=\s+(\d+)', content)
    is_minimization = bool(re.search(r'imin\s*=\s*1', content, re.IGNORECASE)) or \
                      bool(re.search(r'FINAL RESULTS', content))
    if not nstep_matches and is_minimization:
        # Minimization table: header "NSTEP   ENERGY   RMS   GMAX   NAME   NUMBER"
        # data rows: "   5000  -1.78234E+05  ..."
        min_steps = re.findall(r'^\s{1,8}(\d+)\s+[-\d.eE+]+\s+[-\d.eE+]', content, re.MULTILINE)
        if min_steps:
            nstep_matches = min_steps  # reuse same flow below
    ...
    if nstep_matches:
        ...
        # Minimization completed but step count not parseable — treat as INFO not FAIL
        result["checks"].append({
            "check": "nstep_reached",
            "status": "INFO",
            "detail": "Minimization mdout — step count not parseable from this format. Check rst7 exists.",
        })
    else:
        result["checks"].append({
            "check": "nstep_reached",
...
def validate_tleap(log_file):
    ...
    if exit_match:
        ...
        # Categorize warnings
        long_bonds = re.findall(r'bond of ([\d.]+) angstroms between .* atoms:\r?\n-------\s+(.+)', content)
        ...
        _std_res = STANDARD_AA | {"HOH", "WAT", "NA", "CL", "K", "MG", "CA", "ZN", "FE", "MN", "NI", "CU",
                                   "ASH", "GLH", "LYN", "CYM", "HID", "HIE", "HIP", "CYX",
                                   # Lipid21 residue names (split POPC/POPE/POPG etc.)
                                   "PA", "PC", "OL", "PE", "PG", "PI", "PS", "SM", "CHL"}
        ligand_bond_warnings = []
        protein_bond_warnings = []
        for dist, atoms in long_bonds:
            # H-03 fix: extract residue names from tLEaP .R<RESNAME N> format first.
            # Token-based extraction incorrectly flags atom names (CB, CG, ND1 etc.) as ligand tokens.
            res_names = re.findall(r'\.R<([A-Z0-9]{1,4})\s', atoms)
            if res_names:
                is_ligand = any(t not in _std_res for t in res_names)
            else:
                # Fallback: only use uppercase alpha tokens (not digits), skip short tokens
                tokens = re.findall(r'\b([A-Z]{3,4})\b', atoms)
                is_ligand = any(t not in _std_res for t in tokens)
            if is_ligand:
...
def generate_equil_density_script(output_path, prmtop, rst_in, rst_out,
    ...
    M-34: `mdin_path` accepts either a directory or a file path. If a file
    path is given, its parent directory is used. CLI flag is --mdin-dir,
    semantically a directory.
    """
    p = Path(mdin_path)
    # If user passes a file path, take parent. If no suffix, treat as directory.
    mdin_dir = p.parent if p.suffix else p
...
"""
    ...
    script_lines.append('    read STATUS MEAN FLUCT <<< $(parse_density "$OUT")')  # M-35: quote $OUT
