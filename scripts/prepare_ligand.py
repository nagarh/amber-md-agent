...
def align_ligand_to_crystal(crystal_pdb, pubchem_sdf, output_sdf, ligand_code=None):
        ...
        if ligand_code:
            ...
            if lines:
                ...
                try:
                    ...
                    # M-42: ensure tempfile cleanup even on parse error
    ...
    # M-43: verify output was actually written
    if not Path(output_sdf).exists() or Path(output_sdf).stat().st_size == 0:
        return {"error": f"SDF write failed or produced empty file: {output_sdf}"}
