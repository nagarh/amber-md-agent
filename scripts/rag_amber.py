...
class PageIndex:
    ...
    def ingest(self, file_path):
        ...
        for raw in raw_pages:
            ...
            # Track TOC entries — L-08: dedupe against full TOC, not just last 3
            if raw.get("section") and raw["section"] not in {t.get("section") for t in self.toc}:
                self.toc.append({
                    "chapter": raw.get("chapter", ""),
                    "section": raw["section"],
                    "page": raw["page"],
    ...
    def _recompute_idf(self):
        ...
        for token, freq in self.df.items():
            # L-07: clamp IDF at 0 — corpus-wide tokens should not get negative weight
            self.idf[token] = max(0.0, math.log(self.n_pages / (1 + freq)))
    ...
    def save(self, path):
        ...
        # M-39: atomic write (tmp + rename) to avoid corrupt JSON on interrupt
        import os as _os
        tmp = str(path) + ".tmp"
        Path(tmp).write_text(json.dumps(data))  # No indent — save space for large manuals
    ...
    def load(self, path):
        ...
        # M-38: trust actual page count, not the saved n_pages (handles truncated saves)
        self.n_pages = len(self.pages)
        self._recompute_idf()
...
Examples:
    ...
    if args.cmd == "ingest":
        ...
        index = PageIndex()
        index.load(args.index)
        # M-40: validate range format clearly
        try:
            parts = args.range.split("-")
            if len(parts) != 2:
                raise ValueError(f"Expected 'start-end' format, got: {args.range!r}")
            start, end = int(parts[0]), int(parts[1])
        except ValueError as e:
            print(f"ERROR: invalid --range {args.range!r} — {e}")
            return 1
